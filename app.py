...
def generate_map(table_name, selected_date):
    ...
    max_cases = df['confirmed_cases'].max() if 'confirmed_cases' in df.columns else 0
    ...
    for _, row in df.iterrows():
        ...
        size = (cases / max_cases) * 15

        folium.CircleMarker(location=[lat, long], radius=size, popup=popup_text, color=color, fill=True, fill_opacity=0.7).add_to(m)
