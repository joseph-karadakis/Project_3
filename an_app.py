...
def get_confirmed_data(conn, country='US'):
    query = f"SELECT * FROM confirmed WHERE `Country_Region` = '{country}';"
...
def index():
    ...
    countries = conn.execute("SELECT DISTINCT `Country_Region` FROM confirmed;").fetchall()
    conn.close()

    return render_template('an_index.html', countries=countries)
